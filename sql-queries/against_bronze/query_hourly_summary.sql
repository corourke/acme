-- Hourly sales summary, standalone bronze query
SELECT
    DATE_TRUNC (
        'hour',
        CAST(
            parse_datetime (scan_datetime, 'yyyy-MM-dd HH:mm:ss') AS TIMESTAMP
        )
    ) AS sales_day_hour,
...
FROM
    ...
    INNER JOIN acme_retail_bronze.retail_item_master_ro AS item_master ON scans.item_upc = item_master.item_upc
